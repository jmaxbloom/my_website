...
myStocks_returns_total%>%
  summarize(PV_Return = sum(weight*return),beta = sum(weight*beta),alpha = (PV_Return - .015)-beta*(PV_Return-.256)) %>% kableExtra::kable(output = "html",color = "black", background = "lightgrey") %>% kableExtra::kable_classic()%>%kableExtra::column_spec(c(1:3), color = "Black", background = "lightgrey")%>%kableExtra::row_spec(0,background = "lightgray")
